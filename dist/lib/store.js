...
var _version = require('./helpers/version');
...
let VersionStore = class VersionStore {
  get type() {
    return null;
  }

  constructor() {
    let id = (0, _helpers.suid)();
  ...
  // Methods.
  get(name_version) {
    (0, _assert2.default)((0, _helpers.typeOf)(name_version) === 'string', this.errinfo.MSG_VER_NAME);

    let { name, version, path } = (0, _version.split)(name_version);
    ...
    if (this.versions[name]) {
      ...
      if (version) {
        let ver = (0, _version.match)(versions, version);
        ...
        let pkg = this.versions[name][(0, _version.latest)(versions)];
  ...
  add(svc_name, value, version = '0.0.1') {
    if ((0, _helpers.typeOf)(svc_name) === this.type) {
      let { name, version } = svc_name;
      return this.add(name, svc_name, version);
    } else {
      (0, _assert2.default)(typeof svc_name === 'string', this.errinfo.MSG_VER_NAME);
      (0, _assert2.default)((0, _helpers.typeOf)(value) === this.type, this.errinfo.MSG_VAL_TYPE);
      (0, _assert2.default)(typeof version === 'string', this.errinfo.MSG_VER_TYPE);
  ...
  del(namevsn) {
    (0, _assert2.default)((0, _helpers.typeOf)(namevsn) === 'string', this.errinfo.MSG_VER_NAME);

    let { name, version } = (0, _version.split)(namevsn);

    if (this.versions[name]) {
      if (!version) {
        version = (0, _version.latest)(Object.keys(this.versions[name]));
      }
